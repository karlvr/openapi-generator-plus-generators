...
import { each, indent } from '@openapi-generator-plus/template-utils'
...
export function nestedModels(generatorContext: CodegenGeneratorContext, scope: { schemas: CodegenSchemas | null }): string {
	if (!scope.schemas) {
		return ''
	}

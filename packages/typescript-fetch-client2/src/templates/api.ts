...
export function ${identifier(gen, op.name)}ParamCreator(${parameterList(generatorContext, op, groupName)}options: RequestInit = {}, configuration?: Configuration): FetchArgs {
	...
	${op.requestBody ? validateParameter({ parameter: op.requestBody, operation: op, parameterPrefix: '', generatorContext }) : SKIP}
...
function renderResponses(generatorContext: CodegenGeneratorContext, ctx: ApiTemplateContext, op: AnnotatedOperation, hooks: FetchClient2Hooks): string {
	...
	const responseFn = hooks.apiResponseContent
		? (content: CodegenContent | null, response: CodegenResponse) => hooks.apiResponseContent!({ content, response, operation: op, group, rootContext: ctx, generatorContext })
 ...
 */
function renderAcceptHeaderBlock(generatorContext: CodegenGeneratorContext, op: AnnotatedOperation): string | Skip {

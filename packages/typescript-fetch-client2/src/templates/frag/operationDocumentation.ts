...
import { ts, md, indent, indentTail, each, when, maybe } from '@openapi-generator-plus/template-utils'
...
${each(op.parameters, (p) => ` * ${indentTail(parameterDocumentation(generatorContext, p), ' *   ')}`, '\n')}
${when(op.requestBody?.nativeType, () => ` * ${indentTail(parameterDocumentation(generatorContext, op.requestBody!), ' *   ')}`)}

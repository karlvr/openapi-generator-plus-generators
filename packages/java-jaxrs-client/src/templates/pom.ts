...
import { MavenOptions, JavaModelContext } from '@openapi-generator-plus/java-jaxrs-generator-common'
...
/**
 * This generator's `pom.xml`: the JAX-RS/CXF client, Jackson and Swagger dependencies common
 * to every CXF-based client variant. Typed against the family-wide {@link JavaModelContext}
 * (matching the `pom` hook's bag signature) — none of this generator's own root-context
 * fields are needed to render it.
 */
...
<jakarta.xml.bind.version>${mavenVersion(maven, 'jakarta_xml_bind', '3.0.1')}</jakarta.xml.bind.version>`)}
	...
	<dependencies>
		...
		${lombokDependency}
		${ctx.templates.pomDependencies(ctx)}
	</dependencies>
	...
	<dependencyManagement>
		<dependencies>
			...
			${ctx.templates.pomDependencyManagement(ctx)}
		</dependencies>
	</dependencyManagement>
